...
def make_prediction(df, model, scaler, feature_cols):
    ...
    if last_date.normalize() >= today:
        ...
        # If data is old, next trading day is after last_date
...
def generate_trading_guide(prediction_data, df):
    ...
    recent_volatility = df["Daily_Return"].tail(10).std() * 100
    ...
    if change_pct > 2:
        guide.append("🟢 **STRONG BUY SIGNAL**: Model predicts significant upward movement (>2%)")
        guide.append("   - Consider entering a long position")
        guide.append("   - Set stop-loss 2-3% below current price")
        guide.append(f"   - Target price: ${predicted_price:.2f}")
    elif change_pct > 0.5:
        guide.append("🟢 **MODERATE BUY**: Model predicts positive movement (0.5-2%)")
        guide.append("   - Good opportunity for short-term gains")
        guide.append("   - Use tight stop-loss orders")
    elif change_pct < -2:
        guide.append("🔴 **STRONG SELL SIGNAL**: Model predicts significant downward movement (<-2%)")
        guide.append("   - Consider exiting long positions")
        guide.append("   - Opportunity for short selling (advanced traders)")
        guide.append("   - Wait for better entry point if planning to buy")
    elif change_pct < -0.5:
        guide.append("🟠 **MODERATE SELL**: Model predicts negative movement (-0.5 to -2%)")
        guide.append("   - Consider taking profits on existing positions")
        guide.append("   - Avoid new long positions")
    else:
        guide.append("⚪ **NEUTRAL/HOLD**: Model predicts minimal movement (-0.5% to 0.5%)")
        guide.append("   - Market consolidation expected")
        guide.append("   - Hold existing positions")
    ...
    # RSI analysis
    if rsi > 70:
        guide.append(f"⚠️ **OVERBOUGHT** (RSI: {rsi:.1f}): Stock may be overvalued")
        guide.append("   - Potential reversal ahead")
        guide.append("   - Consider taking partial profits")
    elif rsi < 30:
        guide.append(f"⚠️ **OVERSOLD** (RSI: {rsi:.1f}): Stock may be undervalued")
        guide.append("   - Potential bounce opportunity")
        guide.append("   - Good entry point for long-term investors")
    else:
        guide.append(f"✅ **RSI NORMAL** ({rsi:.1f}): No extreme conditions")
    
    guide.append("")
    
    # Volatility analysis
    if recent_volatility > 3:
        guide.append(f"⚠️ **HIGH VOLATILITY** ({recent_volatility:.2f}%): Risky conditions")
        guide.append("   - Use smaller position sizes")
        guide.append("   - Wider stop-loss recommended")
        guide.append("   - Consider waiting for stability")
    else:
        guide.append(f"✅ **NORMAL VOLATILITY** ({recent_volatility:.2f}%): Stable conditions")
...
def main():
    ...
    with st.sidebar:
        ...
        selection_mode = st.radio("Selection Mode:", ["Multiple Stocks", "Single Stock"])
        ...
        st.info("Subscribe to receive daily predictions after market close")
